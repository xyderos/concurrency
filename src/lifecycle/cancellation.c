...
void *fourth_routine(void *arg) {
  ...
  // do not do that!
  // Canceling asynchronously would follow the same route as passing a signal
  // into the thread to kill it, posing problems similar to those in CON37-C. Do
  // not call signal() in a multithreaded program, which is strongly related to
  // SIG02-C. Avoid using signals to implement normal functionality. POS44-C and
  // SIG02-C expand on the dangers of canceling a thread suddenly, which can
  // create a data race condition.
...
void cancellation_simulation(void) {
  core = pthread_self();
