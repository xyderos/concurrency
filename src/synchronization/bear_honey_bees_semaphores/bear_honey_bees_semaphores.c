...
some_honeybees(void *arg)
{
	for (;;) {
		...
		printf(
		    "Honeybee %ld gathered some honey, the pot now is %.6f full\n",
		    (long)arg,
		    (double)((float)current_size_of_the_pot /
			(float)pot_capacity));
		...
		if (current_size_of_the_pot == pot_capacity) {
			...
			printf(
			    "The bees are not done filling up the pot yet\n");
			// increase it indicating that you still need to work
			// until it is full
			sem_post(&bees_are_gathering);
		}
	}
...
int
bear_honey_bees_semaphores(void)
{
	pthread_attr_t attr;
	long i = 0;
