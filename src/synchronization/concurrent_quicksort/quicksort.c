...
static unsigned int
partition(int *arr, unsigned int low, unsigned int high, unsigned int pivot)
{
...
static void
sequential_quicksort(args_t *arguments)
{
	...
	if (arguments->low < arguments->high) {
		unsigned int pivot_position = arguments->low +
		    (arguments->high - arguments->low) / 2;
...
quicksort(void *arg)
{
	...
	if (arguments->low < arguments->high) {
		...
		if (arguments->depth > 0) {
			...
			pthread_create(&thread, NULL, quicksort_worker,
			    &first_half);
