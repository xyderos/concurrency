...
father(void *arg)
{
	...
	for (;;) {
		...
		if (!no_worms_in_the_bowl) {
			...
			// fill in the plate
			for (int i = 0; i < NOF_WORMS; i++)
				put_a_worm_in_the_bowl(i);
...
birds(void *arg)
{
	...
	for (;;) {
		...
		// empty bowl, signal and wait to be filled by father (needs to
		// unlock the mutex since the father will use that and we need
		// to prevent deadlock)
		printf(
		    "There are no worms in the bowl, The father must be notified\n");
		...
		// get the next position of available worn and eat it(the
		// decrement is happening on the get function by changing the
		// position of the next available worm)
		temp = eat_a_worm_from_the_bowl();

		printf("Baby bird %ld just ate worm %d from the bowl\n", id,
		    temp);

		// there is no empty space for the birds to eat so emptySpace
		// will signal the father to refill the plate
		if (position == (NOF_WORMS - 1))
			no_worms_in_the_bowl = 0;

		// unlock
		sem_post(&lock);

		// trying to avoid starvation to other threads since by eating
		// and going to sleep they will be blocvked and the next thread
