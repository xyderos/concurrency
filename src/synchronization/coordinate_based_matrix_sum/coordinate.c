...
int
coordinate_based_matrix_sum(void)
{
	...
	for (l = 0; l < number_of_workers; l++) {
		pthread_create(&workerid[l], &attr, coordinate_worker,
		    (void *)l);
	}
...
void *
coordinate_worker(void *arg)
{
	unsigned long myid = (unsigned long)arg, first = 0, last = 0, i = 0,
		      j = 0, total = 0;
	...
	for (i = first; i <= last; i++) {
		for (j = 0; j < matrix_size; j++) {
			...
			// we might have been swapped from running and the
			// actual value might have been changed
